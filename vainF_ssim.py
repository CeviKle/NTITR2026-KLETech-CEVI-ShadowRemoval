...
class SAMScoreLoss(torch.nn.Module):
    def __init__(self):
        super(SAMScoreLoss, self).__init__()
        self.SAMScore_Evaluation = samscore.SAMScore(model_type = "vit_b" )

    def forward(self, pred, gt):
        self.pred = pred
        self.gt = gt

        samscore = self.SAMScore_Evaluation.evaluation_from_torch(gt, pred)
        print(samscore)
        samscoreloss = 1-samscore
        print(samscoreloss)

        return samscoreloss.mean()
